...
FontOptim.prototype = {
    ...
    if ( /-thin/i.test(filename)            || /Thin/i.test(filename))                     return 100;
    if ( /-light/i.test(filename)           || /Light/i.test(filename) )                   return 300;
    if ( /-medium/i.test(filename)          || /Medium/i.test(filename) )                  return 500;
    if ( /-semi-{0,1}bold/i.test(filename)  || /Semi{0,1}bold/i.test(filename) )           return 600;
    if ( /-bold/i.test(filename)            || /Bold/i.test(filename) )                    return 700;
    if ( /-extra-{0,1}bold/i.test(filename) || /Extra{0,1}bold{0,1}Bold/i.test(filename) ) return 900;
